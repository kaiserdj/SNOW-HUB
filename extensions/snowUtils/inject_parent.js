if (!window.__snu_inject_parent_loaded) {
window.__snu_inject_parent_loaded = true;

setTimeout(function () { //be sure dompurify is loaded
    //Initialize Alert
    var parent = document.querySelector('body');
    var alertContainer = document.createElement('div');
    if (parent && typeof DOMPurify !== 'undefined') {
        alertContainer.innerHTML = DOMPurify.sanitize('<input id="sn_gck" type="hidden" value="' + (typeof g_ck !== 'undefined' ? g_ck : '') + '" /><div style="display:none" class="notification-container service-now-util-alert" role="alert" ><div class="notification outputmsg outputmsg_has_text" style="top: 20px;"><span class="outputmsg_text role="alert"></span></div></div>');
        parent.insertBefore(alertContainer, parent.firstChild);
    }
}, 1500);


window.snuAddDblClickToPin = function snuAddDblClickToPin(cntr = 0) {
...
window.snuAddStudioLink = function snuAddStudioLink() {
...
window.waitForEl = function waitForEl(selector, callback) {
