...
chrome.runtime.onMessage.addListener(function (request, sender, sendResponse) {
    ...
    } else if (request.method == "snuShowSlashCommand") { // Handle slash command from other frames
        if (window.self === window.top) {
            if (typeof snuSlashCommandShow === 'function') {
                snuSlashCommandShow(request.initialCommand, request.autoRun);
            }
        }
...
(function () {
    ...
    chrome.runtime.sendMessage(msg, isServiceNow => {
        if (isServiceNow) {
            if (!window.__snu_purify_loaded) addScript('/js/purify.min.js', false); 
            if (!window.__snu_inject_loaded) addScript('inject.js', true);
            else {
                // Manually trigger settings loading if inject.js was already injected by ViewManager
            ...
            if (window.self === window.top) {
                if (!window.__snu_inject_parent_loaded) addScript('inject_parent.js');
            }
            if (location.pathname.startsWith("/now/") || location.pathname.startsWith("/x/")) {
                if (!window.__snu_inject_next_loaded) addScript('inject_next.js', false);
            }
