if (!window.__snu_inject_next_loaded) {
window.__snu_inject_next_loaded = true;

window.SnuNextManager = class SnuNextManager {
    "use strict";
    constructor() {
        document.addEventListener('dblclick', evt => {
            ...
            if (eventPath[0]?.className && !eventPath[0].className.includes('snuelm'))
        ...
        document.addEventListener('mouseup', evt => {
            ...
            try {
                if (eventPath[0].tagName == "svg") return;
                if (eventPath[0]?.className && eventPath[0].className.includes('sn-polaris-tab')) { //save a click, select input
        ...
        setTimeout(() => {
            if (window.querySelectorShadowDom) {
...
grSPC.deleteMultiple();`;
    ...
    linkPickers(cnt) {
        if (!window.querySelectorShadowDom) return;
