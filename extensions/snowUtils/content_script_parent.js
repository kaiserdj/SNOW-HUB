...
function getVars(varstring) {
    ...
    for (var i = 0; i < variables.length; i++) {
        ...
        scriptContent += `try{ 
            var val = window.${currVariable};
            if (typeof val === 'undefined' && '${currVariable}' === 'g_ck') {
                if (typeof InitialState !== 'undefined') val = InitialState.userToken;
                else if (document.getElementsByName('sysparm_ck').length > 0) val = document.getElementsByName('sysparm_ck')[0].value;
            }
            if (typeof val !== 'undefined') document.body.setAttribute('tmp_${currVariable.replace(/\./g, "")}', val); 
