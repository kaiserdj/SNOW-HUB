if (window.__snu_inject_loaded) {
    if (typeof snuSettingsAdded === 'function') snuSettingsAdded(); // Just re-init settings if loaded twice
} else {
    window.__snu_inject_loaded = true;
...
function snuSettingsAdded() {
    ...
    try { snuLoadThemeVariables(); } catch (e) { console.error('[SN Utils] Error in snuLoadThemeVariables', e); }
    try { snuSetShortCuts(); } catch (e) { console.error('[SN Utils] Error in snuSetShortCuts', e); }

    if (!snusettings.nopasteimage) {
        try { snuBindPaste(snusettings.nouielements == false); } catch (e) { console.error('[SN Utils] Error in snuBindPaste', e); }
    }

    if (snusettings.vsscriptsync == true) {
        try { snuAddFieldSyncButtons(); } catch (e) { console.error('[SN Utils] Error in snuAddFieldSyncButtons', e); }
        try { snuAddStudioScriptSync(); } catch (e) { console.error('[SN Utils] Error in snuAddStudioScriptSync', e); }
        try { snuAddBGScriptButton(); } catch (e) { console.error('[SN Utils] Error in snuAddBGScriptButton', e); }
    }
    ...
    if (snusettings.slashoption != "off") {
        try { snuAddFilterListener(); } catch (e) { console.error('[SN Utils] Error in snuAddFilterListener', e); }
    }
    if (snusettings.s2ify) {
        try {
            if (typeof snuS2Ify != 'undefined') snuS2Ify();
            if (typeof snuNextManager != 'undefined') snuNextManager.linkPickers(0);
        } catch (e) { console.error('[SN Utils] Error in s2ify initialization', e); }
    }
    if (snusettings.nouielements == false) {    
        try { if (typeof snuAddStudioLink != 'undefined') snuAddStudioLink(); } catch (e) {}
        try { if (typeof snuAddDblClickToPin != 'undefined') snuAddDblClickToPin(); } catch (e) {}
        snuAddStudioSearch();
...
function flowDesignerDoubleClick() {
    ...
    document.addEventListener('dblclick', event => {
        ...
        if (angular && path.length > 2 &&
            (path[2].classList?.contains('form-group') || path[2].classList?.contains('content-container'))) {
            let elm = path[2].querySelector('.compounds');
            if (elm) {
...
function snuSlashCommandShow(initialCommand, autoRun) {
    if (window.self !== window.top) {
        // If in iframe, send message to top window instead of direct DOM manipulation
        chrome.runtime.sendMessage({
            method: "snuShowSlashCommand",
    ...
    if (window.top.document.querySelector('div.snutils') != null) {
        ...
        // Improved focus logic for Electron / Polaris
        const focusInput = () => {
            const fltr = window.top.document.getElementById('snufilter');
            if (fltr) {
                window.top.focus();
                fltr.focus();
                fltr.selectionStart = fltr.selectionEnd = 10000;
            }
        };

        focusInput();
        // Secondary focus for edge cases where the first one is captured by shadow DOM
